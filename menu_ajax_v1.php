...
//
//  Verificando session_start - desativado
if( !isset($_SESSION)) {
   session_start();
}
//
// 1. Reportar todos os tipos de erros (Notice, Warning, Error, etc)
error_reporting(E_ALL);
//
// 2. Forçar a exibição desses erros na tela (browser)
...
/**   Final - if( strtoupper($source)=="SAIR" ) { */
//
...
if( $source_maiusc=="LOGAR" ) {
    ...
    */
    ...
    $cmdsql.=" WHERE (a.codigousp=b.codigousp ) and a.codigousp=\"$usuario_conectado\" ";
    $cmdsql.=" order by a.pa  ";
    //   
    $resultado_pa=$conex->query($cmdsql);
    if( ! $resultado_pa  ) {
        $msg_erro .= "SELECT participante/pessoa: ".mysqli_error($_SESSION["conex"]);
        $msg_erro .= $msg_final;
        echo $msg_erro;
        exit();  
    }
    ...
    if( intval($regs)>1 ) {
        ...
        <?php
           //
           // 1. Alteramos mysql_fetch_array para
           //  mysqli_fetch_assoc (mais limpo para nomes de colunas)
           // Note que agora usamos fetch_assoc() que 
           // retorna apenas os nomes das colunas
           while( $linha = $resultado_pa->fetch_assoc() ) {        
                  //
                  ...
                  foreach( $array_pa as $chave => $valor) { 
                         ...
                         if( $valor == $codigo_pa) {
                              ...
                              //
                              // Sua lógica de encoding continua igual, 
                              // mas lembre-se: se o banco for UTF-8, isso é opcional.
                              echo (mb_detect_encoding($campo_nome, 'UTF-8', true) !== 'UTF-8') 
                                    ? htmlentities($campo_nome) 
                                   : $campo_nome;
                              //
                              echo "&nbsp;</option>";
                              //
           ...
           /**  Final - while( $linha = $resultado_pa->fetch_assoc() ) {   */ 
           //
       ?>
        </select>
        </span>
        ...
        <?php
         ...
         $permit_pa=mysqli_result($resultado_pa,0,"pa");    
         //
...
/**  Final - if( in_array(strtolower($source),$menu_opcoes) ) {   */
// 
