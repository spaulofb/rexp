 ...
 */
// Caso SESSION_START desativada - Aativar 
if(!isset($_SESSION)) {
   session_start();
}
//
...
function enviar_dados_con(source,val,string_array) {  
 ...
 alert(" consultar_usuario_js/34  --  source = "+source+"  --  val = "+val+"  --  string_array = "+string_array);
	...
	if( source_maiusc.search(/DESCARREGAR|SELECIONAR/ui)!=-1 ) {  
         //
	...
	/**  Final - if( source_maiusc.search(/DESCARREGAR|SELECIONAR/ui)!=-1 ) {   */
	//


 alert(" consultar_usuario_js/183  -->>  LINHA183  source = "+source+"  --  val = "+val+"  --  string_array = "+string_array);



	//
    if( ( typeof(string_array)!='undefined') && ( source_maiusc=="CONJUNTO" ) ) {
		  //
    	  var m_array = string_array.split("|");
          ...
          if( typeof(m_array)=="object" &&  total_lenght ) {
				...
				if( source.toUpperCase()=="CONJUNTO" ) {
					...
					/**  Final - for( z; z<=total_lenght; z++ ) {  */
					//
				}  
				/**  Final - if( source.toUpperCase()=="CONJUNTO" ) { */
				//
	      }
	...
	/**  Final - if( ( typeof(string_array)!='undefined') && ( source_maiusc=="CONJUNTO" ) ) { */
	//


 alert(" consultar_usuario_js/243  -->>  LINHA243  source = "+source+"  --  val = "+val+"  --  string_array = "+string_array);
	...
	*/
	if( source_maiusc.search(/CORESPONSAVEIS|COLABS/ui)!=-1 ) {  
		//	   
	...
	/**  Final - if( source_maiusc.search(/CORESPONSAVEIS|COLABS/ui)!=-1 ) {   */
	//
    ...
    if( ( pos_autor!=-1 ) && ( pos_val!=-1 ) ) {   
         ...
         if( val.toUpperCase()=="AUTOR" || val.toUpperCase()=="ANO"  ) {      
            ...
            if( document.getElementById('arq_link') ) {
				//
                var lcelem=document.getElementById('arq_link');
			...
			/**  Final - if( document.getElementById('arq_link') ) { */
			//
			...
			if( document.getElementById("tr_opcoes") ) {
				...
				// 
                if( document.getElementById("novo_td_anotacao") ) {
                     referencia.removeChild("novo_td_anotacao");  
				} 
				//
			}  
			/**  Final - if( document.getElementById("tr_opcoes") ) {  */
			//
	 	 }
		 /**    Final - if( val.toUpperCase()=="AUTOR" || val.toUpperCase()=="ANO"  ) {  */
		 //
...
alert(" linha562 -->> poststr = "+poststr+"  -->> source = "+source+"  -->> val = "+val+"  -->> string_array = "+string_array);
	
	//  Enviando os dados para o arquivo php
	myConn.open("POST",raiz_central+receber_dados,true);
	myConn.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	myConn.onreadystatechange=function() { inclusao(myConn); };
	myConn.send(poststr);	
   ...
   alert(" usuario_consultar_js/341 -->> inclusao -->>  pos = "+pos+"  -->>  source = "+source+" - m_dados_recebidos = "+m_dados_recebidos);
                      ...
                      /**  Final - if( pos!=-1 ) {  */
                      //
					  /**  Definindo variavel Maiuscula */
					  var sourceup = source.toUpperCase();


   alert(" usuario_consultar_js/341 -->> inclusao -->> PARTE 1)  -->>  source = "+source
