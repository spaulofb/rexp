...
function  cad_dados_anotacao(source,val,string_array) {
   //
...
// alert(" anotacao_cadastrar_nova.js/7 -  source = "+opcao+" -- val = "+val+" -  string_array = "+string_array);
	...
	if( opcao=="ANOTACAO" ) { 
		...
		if( enviar==1 ) {
			  ...
			  poststr=poststr+encodeURIComponent(val)+"&cip="+encodeURIComponent(m_array);
		}
		if( voltar==1 ) return;
	}
...
// alert("LINHA 240 - m_id_name = "+m_id_name+" - m_id_value = "+m_id_value+" -  m_id_type = "+m_id_type+" - m_erro = "+m_erro)
  ...
  //  var receber_dados = "proj_exp_ajax.php";
    var receber_dados = "../includes/anotacao_cadastrar_ajax_nova.php";
    //
    
//   alert(" anotacao_cadastrar_nova.js/336 -  source = "+opcao+" -- val = "+val+" -  string_array = "+string_array+" - poststr = "+poststr);
    ...
    //
	 var inclusao = function (oXML) { 
                  ...
                  var m_dados_recebidos = trim(oXML.responseText);
  	              document.getElementById('label_msg_erro').style.display="none";
                  if( document.getElementById("tr_projeto_autor_nome") ) document.getElementById("tr_projeto_autor_nome").style.display="none";                         
// alert(" anotacao_cadastrar.js/340 -  source = "+opcao+" -- val = "+val+" -  string_array = "+string_array+" \r\n\n m_dados_recebidos =  "+m_dados_recebidos);                         
    		              ...
    		              //  if( ( val.toUpperCase()=="AUTOR" ) &&  msa.length>=1 ) {
                           if( ( val.toUpperCase()=="PROJETO" ) &&  msa.length>=1 ) {
                                  ...
                                  if( pos!=-1 ) {
                                       ...
                                       document.getElementById("nr_anotacao").innerHTML=array_dados[1];                                                                 document.getElementById("sn_altera_complementa").innerHTML=array_dados[2]; 
                                       if( document.getElementById("tr_projeto_autor_nome") ) {
                                             document.getElementById("tr_projeto_autor_nome").style.display="block";
                                             if( document.getElementById("span_proj_autor_nome") ) document.getElementById("span_proj_autor_nome").innerHTML=array_dados[3];
                                       }
								  }
							} else if( ( val.toUpperCase()=="ALTERA_COMPLEMENTA" ) && msa.length>=1 ) {
                                      ...
                                      document.getElementById('corpo').style.display="block";
         					 	      document.getElementById('label_msg_erro').style.display="block";
                                      if( pos!=-1 ) {
                                          document.getElementById('altera_complementa').options[2].selected=true; 														   document.getElementById('label_msg_erro').innerHTML=m_dados_recebidos;
					            	  } else {
...
function n_coresponsaveis(field,event) {
        ...
        if( !(tecla >= 48 && tecla <= 57 ) && !( tecla==13  ||  tecla==9 ) ) {
           if( !(tecla >= 96 && tecla <= 105 )  ) {
              /* A propriedade keyCode revela o c?digo ASCII da tecla pressionada. 
                Sabemos que os n?meros de 0 a 9 est?o compreendidos entre 
                os valores 48 e 57 da tabela referida. Ent?o, caso os valores 
                n?o estejam(!) neste intervalo, a fun??o retorna falso para quem
