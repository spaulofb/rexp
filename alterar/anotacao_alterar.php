        ...
        if( !is_uploaded_file($_FILES['relatext']['tmp_name']) ) {
              ...
              if( isset($_SESSION["relatext_orig"])  ) {
                   if( strlen(trim($_SESSION["relatext_orig"]))>2 ) {
                       ...
                       if( file_exists(utf8_decode("$dir_relatext_orig")) ) {
                             /// unlink($dir_relatext_orig);
                              unlink(utf8_decode("$dir_relatext_orig"));
                       }
                   }
              }
          ...
          ***/
          ...
          ///
           ...
           if( ! $success ) {
                ...
                $_SESSION["msg_upload"] .=$msg_txt_erro_final;            
                ///
                /// Desativando essa SESSION anotacao_cip_altexc
...
///  alert(" anotacao_alterar.php/531 --->>  AQUI  -->> 1) idselecproj = "+idselecproj+" -- 2)  idopcao  =  "+idopcao+" -  3) string_array = "+string_array);
...
////  alert("AQUI LINHA/513  <<<<<<<<  idselecproj_pos  = "+idselecproj_pos);    
 ...
 ////  alert("anotacao_alterar.php/326 -   cpo#"+i+"  -->> nome="+m_id_name+"  -->> tipo="+m_id_type+"   -->>  valor="+frm.elements[i].value+"  -->>  Total =  "+m_elements_total);
        ...
        ///  Verificando se houve ERRO - retornar falso
         if( m_erro==1 ) {
              return false;
         }
    }  
          ...
          if( lnip==-1 ) {
              if( idselecproj=="DESCARREGAR" ) {
                  ...
                  ///
                   ...
                   return;
                    ...
                    if( pos!=-1 ) {
                        ...
                        if( document.getElementById("autor") ) {
                             document.getElementById("autor").value=partes[0];   
                        }
                        if( document.getElementById("anotador_nome") ) {
                            document.getElementById("anotador_nome").innerHTML=partes[1];   
                        }
                    }   
                   ...
                   /***  Recebendo o Nr. Projeto, Nr. Autor Projeto,  
                   *         nome do arquivo em PDF e Nr. da Anotacao
                   */
...
///  alert("anotacao_alterar.php/641 -idselecproj.toUpperCase() = "+idselecproj.toUpperCase())
 ...
 ///  alert("Ativando final - srv_alteraranot com poststr="+poststr)
    ...
    ///  Envaindo dados para o arquivo AJAX 
    xAJAX_alteraranot.connect(srv_php, "POST", poststr, fndone_alteraranot);   
    ///
}
/*********  Final -  Cadastro da Anotacao de um  PROJETO   *********/
///
</script>
<?php
...
<!--  Final - DIV abaixo para enviar o ARQUIVO da ANOTACAO   -->
        ...
        if( isset($_SESSION["anotacao_cip_altexc"]) ) {
         ...
         if( strlen(trim($cip_anotacao_escolhida))>0 ) {
                ?>
                 <script  type="text/javascript">
                     /* <![CDATA[ */
                     consulta_alteraranot('busca_proj','<?php echo $cip_escolhido;?>','<?php echo $cip_escolhido;?>');
                    /* ]]> */
                 </script>
              <?php
         }
...
<select name="busca_proj" id="busca_proj"   class="Busca_letrai"  title="Selecione o Projeto para Busca de Anota&ccedil;&otilde;es"  onchange="javascript: consulta_alteraranot(this.id, this.value,this.value);"  >
...
<p class='titulo_usp' style='margin: 0px; padding: 2px 0px 0px; line-height:normal;' >Lista de Anota&ccedil;&otilde;es desse Projeto</p>
<?php
} else {
