...
if( $opcao_maiusc=="DESCARREGAR" )  {
    ...
    //  Anotacao
    $n_anotacao=$val;
    //  Caso enviado por anotacao_nova
    if( $n_anotacao=="ANOTACAO" ) {
...
/*    
 $msg_text_erro =" srv_alteraranot.php/149 -  \$testemunha1 = $testemunha1  -  \$testemunha2 = $testemunha2  <br> "
                ." \$titulo_projeto = $titulo_projeto -- \titulo_anotacao = $titulo_anotacao  <br>"
                ." CONTINUAR DAQUI - \$projeto_titulo_parte = $projeto_titulo_parte - \$cip = $cip - \$n_numero = $n_anotacao <br>"
                ." \$_SESSION[nome_autor_projeto] = {$_SESSION["nome_autor_projeto"]}  ";
 
  echo $funcoes->mostra_msg_erro($msg_text_erro);   
  exit();
*/  
   // CONTINUACAO Abaixo  do  if( $opcao_maiusc=="DESCARREGAR" )  {  
 ?>
<!--   <form name="form_alterar_anotacao" id="form_alterar_anotacao"  enctype="multipart/form-data"  method="post"  
...
<textarea cols="90" rows="3"  name="projeto_titulo"  disabled="disabled" style="background-color: #FFFFFF; color: #000000;" ><?php echo nl2br($projeto_titulo);?></textarea>         
   ...
   <tr id="tr_projeto_autor_nome" class="td_inicio1" style="vertical-align: middle; text-align: justify;  margin: 0px; border: none;"  >
    <td id="td_projeto_autor_nome" class="td_inicio1" style="vertical-align: middle; text-align: left; border: none;"   >
       <label >Orientador desse Projeto:&nbsp;</label>
         <span class='td_inicio1' id="span_proj_autor_nome" style=" vertical-align: middle; text-align: left; border:none;  background-color:#FFFFFF; color: #000000;" ><?php echo  $_SESSION["nome_autor_projeto"];?></span>
   </td>
   </tr>
  </table>  
   ...
   <tr style="text-align: left;"  >
     <td  class="td_inicio1" style="text-align: left;" colspan="2"  >
      ...
      <br />
        ...
        ?>
        <select  class="td_select" name="testemunha1" id="testemunha1"  onfocus="javascript: document.getElementById('label_msg_erro').style.display='none';" title="Código da Testemunha (1) da realiza??o " >
        <?php
         ...
         <table border="0" cellpadding="0" cellspacing="0" align="center" style="width: 100%; line-height: 0px; margin:0px; border: none; vertical-align: top; " >
            <tr style="border: none;">
               <td  align="CENTER" nowrap style="text-align:center; border:none;" >
                  <button name="cancelar" id="cancelar"  type="button" onClick="javascript: consulta_alteraranot('CANCELAR','LIMPAR');"  class="botao3d" style="cursor: pointer;  width: 120px; "  title="Cancelar"  acesskey="C"  >    
               ...
               <td  align="center"  style="text-align: center; border:none; ">
                  <button type="button" name="alterar" id="alterar"  class="botao3d" onClick="javascript: consulta_alteraranot('SUBMETER','ANOTACAO',this.form);"  style="cursor: pointer; width: 120px; "  title="Alterar"  acesskey="A" >    
...
}  // Final  do  if( $opcao_maiusc=="DESCARREGAR" )  { 
...
if( $opcao_maiusc=="TODOS" )  {
        ...
        */
        ...
        $sql_temp = "DROP TABLE IF EXISTS   ".$_SESSION["table_consultar_anotacao"]."    ";  
        ...
        if( ! $drop_result  ) {
            ...
            echo $msg_erro; */
            echo $funcoes->mostra_msg_erro("&nbsp;Removendo a Tabela  {$_SESSION["table_consultar_anotacao"]} - db/mysql:&nbsp;".mysqli_error($_SESSION["conex"]));
            exit();        
        }
       ...
       */  
       $sqlcmd .= "SELECT a.numero as nr, a.alteraant as Altera, a.alteradapn as Alterada, "
        ...
        $_SESSION["pagina"]=0;  $_SESSION["m_function"]="consulta_alteraranot" ;  
        // 
        if( isset($_SESSION["anotacao_cip_altexc"]) ) {
            ...
            $_SESSION["opcoes_lista"] = "../includes/tabela_altera_anotacao.php?pagina=";
            require_once("../includes/tabela_altera_anotacao.php");                      
        }
        ///
        exit();
    ...
    if( strpos($val,"#")!==false ) {
       ...
       if( isset($array_proj_anot) ) {
        ...
        if( isset($m_array) ) {
           if( strpos($m_array,"#")!==false ) $array_proj_anot = explode("#",$m_array);
           if( isset($array_proj_anot) ) {
               $cip=$array_proj_anot[2];
               $anotacao=$array_proj_anot[3];        
           }  
        }
    }
    ...
    $campos = "&nbsp; srv_alteraranot_nova.php/192 -   \$numprojeto = $numprojeto -- \$titulo_projeto  = $titulo_projeto <br> ";
