...
if( $source_upper=="PARTICIPANTE" ) {  
    ...
    if( intval($n_regs)<1  ) {
         ...
         if( intval($n_usu)<1 ) {
              ...
              $partes .="none#".mysql_result($resultado_usuario,0,"senha_usu"); 
         } 
      ...
      if( intval($n_regs)==1 ) {
          ...
          if( intval($n_usu)==1  ) {
               ...
               if( intval($aprovado_usu)<=1 )  {
                   ...
                   $pa_descr_cadastrado=mysql_result($executar,0,0);
  ...
  ////  $m_erro=99999;   ////  Valor  temporario 

/***  
  $temptxt .= " \$campo_nome = ".substr($campo_nome,-50)." <br/> \$campo_value = ".substr($campo_value,-50)."  <br/>  ";
  $temptxt .=   " \$_SESSION[pasta_raiz] = {$_SESSION["pasta_raiz"]} ou  \$_SESSION[pagina_local] = {$_SESSION["pagina_local"]} ";
   $temptxt .=   "( values('$login',password('$senha'),'$datacad','$datavalido',$codigousp,$pa,1,'$activ_code') ";      
  $temptxt .="<br/> \$senha = $senha  ";
  echo  $temptxt;
  exit();      
****/      
    ...
    if( intval($m_erro)<1 ) {
         ...
         $sqlcmd = "SELECT nome FROM $bd_1.pessoa  WHERE codigousp=$codigousp ";
          ...
          $nome = mysql_result($resultado_pessoa,0,"nome");
          ///
         ...
         if( intval($m_erro)==1  ) {
                ...
                if( ! $success ) {
                    ...
                    /***
                       ...
                       $msg_ok .="</p>".$msg_final;
                        ***/
                        $msg_ok .="<br/><span style='color:#F00FFF;padding:.8em 0 .8em 0;font-size:large;'>Participante:&nbsp;"
                                  .$nome." foi cadastrado como ".ucfirst($descr_pa).".<br>";                     
    ...
    ///  Final - Caso NAO houve erro enviar EMAIL
 /***   
  $msg_ok .="<br/><span style='color:#F00FFF;padding:.8em 0 .8em 0;font-size:large;'>Participante:&nbsp;"
                     .$nome." foi cadastrado como ".ucfirst($descr_pa).".<br>";                     
         $msg_ok .="</span>".$msg_final;
