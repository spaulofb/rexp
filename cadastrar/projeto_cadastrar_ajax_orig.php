...
if( ( $source_upper=="CORESPONSAVEIS" ) or  ( $source_upper=="COLABS" ) ) {	
    ...
    */
    ...
    //
      ...
      /**   
    echo "ERRO: projeto_cadastrar_ajax/539  -->> \$m_linhas = {$m_linhas}  ";
    exit();
         */
...
/**
     echo "ERRO: LINHA/562  -->> \$campo_nome = $campo_nome <<--  "
          ."<br>    -->> \$campo_value = $campo_value    ";
     exit();
   */
...
/**
     ...
     echo "ERRO: LINHA/576  -->> \$campo_nome = $campo_nome <<--  "
...
/**
  echo "ERRO: LINHA/819  -->> \$m_regs = $m_regs  <<-->>  \$objetivo = $objetivo  <<--  \$m_datainicio = $m_datainicio  --   \$m_datafinal = $m_datafinal ";
  exit();        
   */
...
/**
     $sumx = count($array_t_value);
     echo "ERRO: LINHA/869  -->>  ANTES \$campo_nome = $campo_nome <<--  "
          ."<br>    -->> \$campo_value = $campo_value   <br> -->> SESSION[numprojeto] = {$_SESSION["numprojeto"]}  "
          ."  -->>  \$count_array_temp = $count_array_temp  -->> \$sumx = $sumx <br>"
          ."  -->> \$m_array =<b> $m_array  -->> \$array_temp = count </b> ".count($array_temp)
...
/**     
  echo "ERRO: LINHA/902  -->> \$m_regs = $m_regs  <<-->>  VER  \$objetivo = $objetivo  <<-- "
       ."  <br>  \$m_datainicio = $m_datainicio  --   \$m_datafinal = $m_datafinal ";
  exit();        
   */  
