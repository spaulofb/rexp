...
if( $source_upper=="TESTEAANDOAPENAS" ) {
    echo "ERRO: cadastar/proj_exp_ajax/244   ";
    exit();
}   
////
...
if(  ( $source_upper=="CORESPONSAVEIS" ) or  ( $source_upper=="COLABS" ) ) {	
  ...
  $m_co = "Co-resp.";
   ...
   /*   Atrapalha e muito essa programacao orientada a objeto
        include('php_include/ajax/includes/class.MySQL.php');
...
if( $source_upper=="ANOTACAO" ) { //  IF  - Tabela ANOTACAO 
    ...
    if( strtoupper(trim($val))=="PROJETO" ) {
            ...
            $result_anotacao=mysqli_query("SELECT  anotacao,cip,numprojeto FROM $bd_2.projeto  where cip=".$cip_codigouso[0]);
           if( ! $result_anotacao ) {
                $msg_erro .= "Select rexp.projeto e anotacao - falha: ".mysql_error().$msg_final;
                echo $msg_erro;
                exit();        
           }
        ...
        ?>
        <label  style="vertical-align:middle;  cursor: pointer; text-align: left; background-color:#FFFFFF; color:#000000;"  title="N&uacute;mero da Anota&ccedil;&atilde;o" ><?php echo $anotacao;?></label>
        <label for="altera_complementa"  style="cursor: pointer; "  title="Altera ou Complementa o Projeto" >Altera/Complementa (Sim/N&atilde;o)?&nbsp;</label>
      <select name="altera_complementa"   id="altera_complementa" class="td_select"  title="Altera ou Complementa o Projeto" onchange="javascript: enviar_dados_cad('anotacao','altera_complementa',this.value)"  >
      <option value="" >== Selecionar ==</option>
        <option value="1,<?php echo $cip_codigouso[0];?>" >Sim</option>
      ...
      <?php
            ...
            if( $array_altera[0]==1 ) {
    			//  Quando usar esse Select necessita do GROUP
	    	/*	$result_altera=mysqli_query("SELECT  anotacao,cip,numprojeto FROM  projeto "
				      ."  where  cip=".$array_altera[1]."  ");  */
					  
					//  Quando usar esse Select necessita do GROUP
    		/*	$result_altera=mysqli_query("SELECT  a.anotacao, a.cip,a.numprojeto, "
			                       ." a.autor,b.nome FROM "
			                       ." projeto a, pessoal.pessoa b where "
								   ."  a.cip=".$array_altera[1]."  and b.codigousp=a.autor   "
								   ." order by a.cip ");	  
                */
                $result_altera=mysqli_query("SELECT  a.cia,a.numero,a.projeto,a.autor,a.data,b.numprojeto FROM "
                                   ." $bd_2.anotacao a, $bd_2.projeto b where "
                ...
                $_SESSION["cia_proj"]=$array_altera[1];
 				if( $x_anot<1 ) {
                      ...
                      echo $msg_erro;
			   		...
			   		?>
                    <label for="alteraant"  style="vertical-align:middle;  cursor: pointer; text-align: left; color:#000000;"  title="Selecione Anotação anterior" >Selecione Anotação anterior:&nbsp;</label>
                    <select name="alteraant" id="alteraant" class="td_select"   title="Altera/complementa uma anota&ccedil;&atilde;o anterior"  >                   
                    <?php
   ...
   unset($val);
   exit();
}  // FINAL  IF  - source Tabela ANOTACAO 
...
if( strtoupper($val)=="ANOTACAO" ) {
    ...
    if( ! $result_anot ) {
          ...
          die('ERRO: Select rexp.projeto e campo anotacao - falha: '.mysql_error());  
     }
     //  CORRIGIDO  sabado - 20110709
    ...
    $sqlcmd="insert into rexp.anotacao(".$_SESSION["campos_nome"].") values(".$_SESSION["campos_valor"].") ";
    ...
    if ( $success ) { 
        ...
        $result=mysqli_query("update $bd_2.projeto set anotacao=$anotacao where  cip=$projeto  ");
        if( $result ) {
            ...
            echo  $msg_ok;
            // Efetiva a transação nos duas tabelas (anotacao e projeto)
            mysqli_query('commit'); 
        } else { 
