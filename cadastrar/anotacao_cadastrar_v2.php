...
if( $_SERVER["REQUEST_METHOD"] == "POST" ) {
    ...
    if( isset($_FILES['relatext']['tmp_name']) ) {
         ...
         if( isset($_POST["autor_cod"]) ) { 
             //
             if( strlen(trim($_POST["autor_cod"]))>0 ) {
                  $autor_cod=$_POST["autor_cod"];  ///  Autor da Anotacao    
             } 
             //
         }
         /**  FInal - if( isset($_POST["autor_cod"]) ) {  */
         //
         //  Conectar  BD
         ...
         */ 
         ...
         $xbdpri = $db_array[$elemento];
         //
         /** Selecionar BD  */
         $db_select = mysqli_select_db($conex,$xbdpri);
         //


    /**  
 echo "LINHA/135  -->> REQUEST_METHOD  -->>  \$xbdpri = $xbdpri  ";
 exit();   
     */
        ...
        /**  Final - if( ! isset($cip) ) { */
        //
        $proc="SELECT numprojeto,autor FROM $bd_2.projeto  WHERE cip=$cip ";
        $select_numprojeto = $conex->query("$proc");
        if( ! $select_numprojeto ) {
             //
             $terr="ERRO: Select projeto campo numprojeto - falha:&nbsp;db/mysqli:&nbsp;";
             $terr.=mysqli_error($_SESSION["conex"]);
              die("$terr");  
              exit();
        }  
        /**  Final - if( ! $select_numprojeto ) { */
        //
        /**
         *  Utilizando function mysqli_result 
         */
        $_SESSION["numprojeto"]=mysqli_result($select_numprojeto,0,"numprojeto");
        $projeto_autor=mysqli_result($select_numprojeto,0,"autor");
        //
        ...
        if( is_array($_FILES['relatext']['tmp_name'])) {
            //
        ...
        /**  Final - if( $tamanho_dir[0]<1  or !file_exists($_SESSION["dir"]) ) { // Tem que ser maior que 8 bytes  */
        //
  	    for( $n=1; $n<3 ; $n++ ) {                
             //
             /**  
        ...
        if( intval($conta_arquivos)>1 ) {
             //
    	       for( $i=0; $i < $conta_arquivos; $i++ ) {
                 $tam_total_arqs += $_FILES["relatext"]["size"][$i];    
             } 
             //
        } else {
             //
        ...
        //
        $ttdirarq = $tamanho_dir[0]+$tam_total_arqs;
	      //
   	    /*** Tamanho maximo no arquivo configurado no  php.ini   
        ...
        */
        ...
        if( $_SESSION["result"]=='OK' ) {
            //
		        ...
		        $espaco_em_disco =  (int) $max_file_size-$tamanho_dir[0];  
		        if( $espaco_em_disco<1024 ) {
                  $tipo_tamanho = "bytes";
            } 
            //
		        if( $espaco_em_disco>1024 and $espaco_em_disco<1024000 ) {
		              $tipo_tamanho = "KB"; 
                  $espaco_em_disco= intval($espaco_em_disco/1024);
	   	      } else if( $espaco_em_disco>=1024000 ) {
	   	             $tipo_tamanho = "MB"; 
                   $espaco_em_disco= intval($espaco_em_disco/1024000);
		        }
		        //
	          if( intval($ttdirarq)>intval($max_file_size)  ) {
                  //
	                $erros='ERRO';
            ...
            if( !is_uploaded_file($_FILES['relatext']['tmp_name']) ) {
                 ...
                 if( ! isset($anotacao_numero) ) {
                    ...
                    if( isset($_SESSION["anotacao_numero"]) ) {
                         //
                         if( strlen(trim($_SESSION["anotacao_numero"]))>0 ) {
                              //
                              $anotacao_numero=$_SESSION["anotacao_numero"];  
                         }
                         // 
                    }
                    //              
                ...
                **/
                ...
                if( @move_uploaded_file($_FILES['relatext']['tmp_name'],$xdir."$filename") ) {    
                    ...
                    $erros='';
                    //
                    $_SESSION["msg_upload"]= $msg_erro."Arquivo: ".$flnnew.' foi armazenado&nbsp;';
                    $_SESSION["msg_upload"].=$msg_erro_final;
         ...
         if( trim($erros)=='' ) {
             ...
             $nupdate = mysqli_query($_SESSION["conex"],"$proc");
		         ...
		         if( ! $nupdate ) {
                 //
                 $_SESSION["msg_upload"]= $msg_erro.'FALHA no armazenamento do arquivo: '.$flnnew.'<br/>';
                 ...
                 $proc="SELECT numero as nanotacao, projeto as nprojeto,autor as autor_anotacao  ";
                 $proc.=" FROM $bd_2.anotacao WHERE  projeto=$nprojexp  and  numero=$anotacao_numero  ";
                 $success=mysqli_query($_SESSION["conex"],"$proc");
                  ///
                  if( ! $success ) {
                      //
                      // Ocorreu ERRO/FALHA no Select da Anotacao do Projeto
                      $_SESSION["msg_upload"]= $msg_erro;
                      $_SESSION["msg_upload"].="Selecionando a Anotação $anotacao_numero do Projeto ";
                      $_SESSION["msg_upload"].="- FALHA:&nbsp;db/mysql:&nbsp;";
                      ...
                      if( intval($nregs)>0 ) { 
                           ...
                           $_SESSION["msg_upload"] .= $msg_erro."Anota&ccedil;&atilde;o $anotacao_numero do Projeto ";
                           $_SESSION["msg_upload"] .= $_SESSION["numprojeto"]."  foi conclu&iacute;da.";
                           $_SESSION["msg_upload"] .=$msg_erro_final;        
                           ///
                      } else {
                          ///  Caso NAO encontrou Anotacao do Projeto
                          $_SESSION["msg_upload"]= $msg_erro."Anotação $anotacao_numero do Projeto ";
                          $_SESSION["msg_upload"].=$_SESSION["numprojeto"]."  não encontrada.";
                          $_SESSION["msg_upload"].=$msg_erro_final;
                      }
                      //
                  } 
    ...
    /**  Final - if( isset($_FILES['relatext']['tmp_name']) ) {  */
    //
}  
...
***/  
require_once("{$_SESSION["incluir_arq"]}js/anotacao_cadastrar_js.php");
///
...
<div  id="arq_link" style="display: <?php echo $_SESSION['display_arq'];?>;"  >
 <form name="form_arq_link" method="POST"  enctype="multipart/form-data" >
    ...
    <tr>
     <!-- Relatorio Externo (link) Projeto -->
	  ...
	  <td class="td_inicio2" style="vertical-align: middle; " colspan="1"  >
        ...
        <input type="file" name="relatext"  id="relatext"  size="90" accept="application/pdf"    
...
<!-- div - ate antes do Limpar e Enviar -->
           ...
           <span>    
                ...
                <?php 
                    ...
                    ***/
                    ...
                    $proc.="  WHERE codigousp=$usuario_conectado  order by nome ";
                    $res_anotador = mysqli_query($_SESSION["conex"],"$proc");
                    ///
                    if( ! $res_anotador ) {
                         //
                         $terr="ERRO: Select pessoal.pessoa - falha: -&nbsp;db/mysqli:&nbsp;";
                         die("$terr".mysqli_error($_SESSION["conex"]));  
                         //
                    }
                    /**  FInal - if( ! $res_anotador ) {  */
                    //
                    ...
                    if( intval($m_linhas)<1 ) {
                        $autor="== Nenhum encontrado ==";
                         ...
                         //
                         $_SESSION["anotador_codigousp"]=mysqli_result($res_anotador,0,"codigousp");
                         ///  IMPORTANTE:  pra acentuacao htmlentities
                         ///  $anotador_nome=mysqli_result($res_anotador,0,"nome");
                         ///  $anotador_nome=htmlentities(mysqli_result($res_anotador,0,"nome"),ENT_QUOTES,"UTF-8");
                         $anotador_nome=mysqli_result($res_anotador,0,"nome");
                         $_SESSION["anotador_nome"]=$anotador_nome;
                         $anotador_categoria=mysqli_result($res_anotador,0,"categoria");   
                         //                 
       ...
       <div class="div_nova" style="margin: .4em 0 .4em 0;" >
        ...
        <?php  
            ...
            */
            ...
            //
            $a1=array("chefe","vice");
            $res1 = array_uintersect($a1, $array_pa, "strcasecmp");
            // 
            ...
            //  if( $res1 ) {
                 ...
                 $sqlcmd.=" a.codigousp=".$_SESSION["usuario_conectado"]." order by b.titulo ";
                 //
            } 
            //     
            //
            $a2=array("anotador");
            $res2 = array_uintersect($a2, $array_pa, "strcasecmp");
            if( $permit_pa==$array_pa["anotador"] ) {
            //  if( $res2 ) {
                 //
                /**  Select/MYSQLI    */
                $sqlcmd = "SELECT a.codigousp,a.nome,b.cip,b.fonterec,b.fonteprojid,b.numprojeto,b.titulo, ";
                $sqlcmd .=" b.anotacao FROM $bd_1.pessoa a, $bd_2.projeto b, $bd_2.anotador c ";
                $sqlcmd .=" WHERE a.codigousp=b.autor and "; 
                $sqlcmd .=" b.cip=c.cip and c.codigo=".$_SESSION['anotador_codigousp']." order by b.titulo ";
                //
            }
            /**  Final - if( $res2 ) {  */
            //
            //
            $a3=array("orientador");
            $res3 = array_uintersect($a3, $array_pa, "strcasecmp");
            if( $permit_pa==$array_pa["orientador"] ) {          
            //  if( $res3 ) {               
                //
                /**  Select/MYSQLI    */
                $sqlcmd = "SELECT a.codigousp,a.nome,b.cip,b.fonterec,b.fonteprojid,b.numprojeto,b.titulo, ";
                $sqlcmd .=" b.anotacao FROM $bd_1.pessoa a, $bd_2.projeto b ";
                $sqlcmd .=" WHERE a.codigousp=b.autor and ";
                $sqlcmd .=" b.autor=".$_SESSION["usuario_conectado"]." order by b.titulo ";  
                //
            }  
            //
            ///  SELECT/MYSQLI
            $rsql=mysqli_query($_SESSION["conex"],$sqlcmd);
            ///                  
            if( ! $rsql ) {
                  //
                  $msg_erro .= "Selecionando os projetos autorizados para esse Anotador - falha&nbsp;db/mysqli:&nbsp;";
    ...
    <?php
       ...
       if( intval($m_linhas)<1 ) {
          ...
          <?php
            //
            $nconta=1;
            while( $linha=mysqli_fetch_assoc($rsql) ) {
                   //
                   ...
                   $partes_antes=6;                     
                   $prj_tit_parte="";  
                   $wordstit = explode(" ",trim($linha['titulo']));
                   $sum_words=count($wordstit);
                   //
                   for( $i=0; $i<$sum_words; $i++ ) {
                        //
                        $prj_tit_parte .="{$wordstit[$i]} ";
                        //
                        if( $i==$partes_antes and $sum_words>$partes_antes  ) {
                             $prj_tit_parte=trim($prj_tit_parte);
                             $tamanho_campo=strlen($prj_tit_parte);
                             if( $tamanho_campo>40  ) $prj_tit_parte.="...";
                             //
                             break;
                             //
                         }
                   ...
                   /**  Final - for( $i=0; $i<$sum_words; $i++ ) {  */
                   //
                   //
                   $titprj="";
                   if( strlen(trim($fonterec))>=1 ) $titprj.= $fonterec."/";
                   if( strlen(trim($fonteprojid))>=1 ) $titprj.= $fonteprojid.": ";
                   //
                   $_SESSION["cip"]=$linha['cip'];
                   $cip=$_SESSION["cip"];
                   //
                   $autor_nome =$linha['nome'];
                   $titprj .= trim($prj_tit_parte);
                   //
                    ...
                    echo  "<option value=".$linha['cip']." title='Orientador do Projeto: $autor_nome' >"
                             .$nconta.")&nbsp;".htmlentities($titprj,ENT_QUOTES,"UTF-8")."&nbsp;&nbsp;</option>";   
                    ///
                    ++$nconta;
                    ///            
              }
              /**  Final - while( $linha=mysqli_fetch_assoc($rsql) ) {  */
              //
            ?>
             </select>
            <?php
        ...
        /**  Final -  Final da Num_USP/Nome Responsavel  */
        //
       ?>  
     </div>
       ...
       <article class="anotacaoarticle1" style="text-align: center; max-width: 99.2%; alignment-adjust: central;"  >
          <table class="table_inicio" style="margin: 0px; "  >
            <tr class="td_inicio1"  >
             ...
             <td id="nr_anotacao"  class="td_inicio1" style="vertical-align: top; background-color:#FFFFFF; color:#000000;"  >
             ...
             <!--  Data da Anotacao  -->
               <td class="td_inicio1" style="vertical-align: middle;text-align: left; background-color:#FFFFFF; color:#000000;"  >
                 &nbsp;Data:&nbsp;<?php echo $_SESSION["data_de_hoje"];?>
               </td>
            </tr>
        </table>
     </article>      
   ...
   <div class="div_nova" >
     <article class="anotacaoarticle1"  >
       ...
       <div>
           <textarea rows="6"  name="titulo" id="titulo" required="required"   
    ...
    <article class="anotacaoarticle1"   >
      ...
      <br />
        ...
        <?php 
            ...
            //
            /***
            *   IMPORTANTE:  Utilizar o charset UTF-8 recomendacao, 
            *    pois cobre quase todos os caracteres e 
            *    símbolos do mundo
            ***/
            $_SESSION["conex"]->set_charset("utf8");
            ...
            $result2=mysqli_query($conex,"{$proc}");
            //
            if( mysqli_error($_SESSION["conex"]) ) { 
                 //
                 $terr="Falha no Select tabela pessoa -";
                 $terr.="&nbsp;db/Mysqli:&nbsp;";
            ...
            //
        ?>
        <select onfocus="javascript: document.getElementById('label_msg_erro').style.display='none';" name="testemunha1" 
               id="testemunha1" class="td_select"  title="Código da Testemunha (1) da realização " >
        <?php
            //
            /**  Código da Testemunha (1) da realização  */
            $testemunhas_result = $result2;
            //
            require_once("{$_SESSION["incluir_arq"]}includes/testemunhas.php"); 
            //
          ?>
          </select>
          <?php
