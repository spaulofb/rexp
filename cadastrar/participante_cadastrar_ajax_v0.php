...
/**
 *   echo  "ERRO: participante_cadastrar_ajax/291  -->> include  <<-- \$source_upper = $source_upper  <br>"
 *               ." -->>  \$n_regs = $n_regs  ";
 *      exit();    
 */
...
/** 
 echo  "ERRO: participante_cadastrar_ajax/318  -->>  \$source_upper = $source_upper  <br>"
                ." -->>  \$proc = $proc  <br> \$partes = $partes  ";
 exit();    
 */
     ...
     // Nr. de registros
     $n_usu = mysqli_num_rows($rsqlusu);
     //

/**
 echo  "ERRO: participante_cadastrar_ajax/331  -->>  \$source_upper = $source_upper  <br>"
                ." -->>  \$n_usu = $n_usu  ";
 exit();    
 */
     ...
     if( intval($n_usu)<1 ) {
         ...
         /**  $partes .="none#".mysqli_result($rsqlusu,0,"senha_usu");   
        *   Codigo do PA
         */
         $xpa=mysqli_result($rsqlusu,0,"pa_usu");
         //
         $elemento=6;
         //  include("/var/www/cgi-bin/php_include/ajax/includes/conectar.php");
...
/**        
 ...
 exit();    
   */ 
      ...
      $rsqlusu=mysqli_query($_SESSION["conex"],"$sqlcmd");                       
      /// Caso houve erro
      if( ! $rsqlusu ) {
           //
           // die('ERRO: SELECT Usu&aacute;rio/Paticipante: '.mysql_error());
 ...
 */
      ...
      if( intval($n_usu)==1  ) {
          //
           ///    $senha_usu=mysql_result($rsqlusu,0,"senha_usu");
           ///    $pa_usu=mysql_result($rsqlusu,0,"pa_usu");
           ///    $aprovado_usu=mysql_result($rsqlusu,0,"aprovado_usu");
           /****
                Definindo os nomes dos campos e valores recebidos do 
                   MYSQL SELECT - mysql_fetch_array - IMPORTANTE
            ***/
           $array_usuario=mysqli_fetch_array($rsqlusu);
...
/**  
  $temptxt = "ERRO:  909) \$bd_1 = $bd_1 e \$bd_2 = $bd_2   <<--<br> \$campo_nome = ".substr($campo_nome,-50)." <br/> \$campo_value = ".substr($campo_value,-50)."  <br/>  ";
         ...
         if( intval($usuario_inserir)==1 ) {
             ...
             */
             $success=mysqli_query($_SESSION["conex"],"INSERT INTO $bd_1.usuario "
                     ...
                     ."  values('$login',old_password('$senha'),'$datacad','$datavalido',$codigousp,$pa,1,'$activ_code') ");
             //                                          
             if( ! $success ) {
                  //
                  //  Ocorreu ERRO
         ...
         if( intval($m_erro)==1  ) {
             ...
             if( ! $success ) {
                ...
                ***/
                ...
                $m_erro=0;
                /**
                 *   NAO UTILIZANDO  SUCESSO ENVIANDO EMAIL 
                 *   APENAS NESSA PARTE
                 */
                echo $msg_ok; 
                ///
