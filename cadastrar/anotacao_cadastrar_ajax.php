<?php
/**
*   Arquivo para CADASTRAR ANOTACAO AJAX -  20250520
*/
//
...
/**  
echo "ERRO: LINHA117  -->>> anotacao_cadastrar_ajax  -->>  Passou do conectar.php   ";
exit();
 */
...
if( $source_upper=="CONJUNTO" ) {
      ...
      if( $n_cpo<=$cpo_final ) {   ///  Inicio do IF principal
         ...
         */
         $pattern = '/^(BLOCO|SALATIPO|SALA)$/ui';
        if( preg_match($pattern, $table_atua)) {
              //
        ...
        */
        //
        //  Select/Mysqli
        $sqlcmd="SELECT $select_cpo, $nome_cpo count(*) FROM  $bd_1.$table_atual ";
        $sqlcmd.=" WHERE $where  GROUP BY 1 ORDER BY $select_cpo ";
        $result=mysqli_query($_SESSION["conex"],$sqlcmd);
        //
        if( ! $result ) {
            //
            die('ERRO: Select tabela $table_atual - falha: '.mysqli_error($conex));
            //
        } 
        //
        ...
        $cp_cpo_where=$cpo_where;
        $ctb=strtoupper(trim($cp_table_atual));
        $cpowr=strtoupper(trim($cp_cpo_where));
        if( $ctb=="INSTITUICAO" ) $cp_table_atual="instituição";
        if( $cpowr=="INSTITUICAO" ) $cp_cpo_where="instituição";
        if( $ctb=="DEPTO" ) $cp_table_atual="departamento";
        if( $cpowr=="DEPTO" ) $cp_cpo_where="departamento";  
        // 
            ...
            -->
             ...
             <?php
              ...
              <?php
                  ...
                  //
                  // Final do SELECT
                  $tvx=strtoupper(trim($_SESSION["select_cpo"]));
                  if( $tvx=="SALA" ) {
...
if( ( $source_upper=="CORESPONSAVEIS" ) or ( $source_upper=="COLABS" ) ) {	
     ...
     if( ! $result ) {
            // die('ERRO: Select pessoa - falha: '.mysqli_error());  
            $msg_erro .= "Select Tabela pessoa - db/mysql:&nbsp;";
...
if( $source_upper=="ANOTACAO" ) { 
    //

/**  
echo  "ERRO: LINHA440  -->>> anotacao_cadastrar_ajax  -->>  \$source_upper = $source_upper "
       ."  -->>  \$val_upper = $val_upper  ";
exit();
 */
    ...
    //
    if( $val_upper=="VERIFICANDO" ) {
         //
        if( $_SESSION["permit_pa"]==$array_pa["orientador"]  )  {
            //
            $usucon=$_SESSION["usuario_conectado"];
            $result_proj=mysqli_query("SELECT  numprojeto FROM $bd_2.projeto  WHERE autor=".$usucon);
            /// Verificando FALHA no MySql SELECT
           if( ! $result_proj ) {
                $msg_erro .= "Select Tabela projeto - falha: ".mysqli_error().$msg_final;
                echo $msg_erro;
                exit();        
           }
           /// Numero de registros
    ...
    if( $val_upper=="PROJETO" ) {
        ...
        /**  
         *  $result_anotacao=mysqli_query("SELECT  a.anotacao, a.cip, a.numprojeto,"
         *       ." b.nome as projeto_autor_nome "
         *       ." FROM $bd_2.projeto a, $bd_1.pessoa b  "
         *       ." WHERE a.autor=b.codigousp  and  cip=".$cip_codigouso[0]);
         */
        $proc="SELECT  a.anotacao, a.cip, a.numprojeto,";
        $proc.="  b.nome as projeto_autor_nome  ";
        $proc.=" FROM $bd_2.projeto a, $bd_1.pessoa b  ";  
        $proc.=" WHERE a.autor=b.codigousp  and  cip=".$cip_codigouso[0];
        $result_anotacao=mysqli_query($_SESSION["conex"],"$proc");
        //
        //  if( ! $result_anotacao ) { 
        if( mysqli_error($_SESSION["conex"]) ) {
            //
            $msg_erro .= "Select Tabelas projeto e anotacao - falha: ";
            $msg_erro .= mysqli_error($conex).$msg_final;
            echo $msg_erro;
            exit();        
        }  
        /**  Final -  if( mysqli_error($_SESSION["conex"]) ) {  */
        //
...
/**  
echo  "ERRO: LINHA515  -->>> anotacao_cadastrar_ajax  -->>  \$source_upper = $source_upper "
       ."  -->>  \$val_upper = $val_upper -->> \$array_nome0 count ".count($array_nome0);
exit();
   */
        ...
        /**  $_SESSION[$anotacao]= (int)  mysqli_result($result_anotacao,0,"anotacao")+1;
           *    $anotacao=$_SESSION[$anotacao]; 
        */
        $anotacao = (int) mysqli_result($result_anotacao,0,"anotacao")+1;          
        //

/**  
echo  "ERRO: LINHA540  -->>> anotacao_cadastrar_ajax  -->>  \$anotacao = $anotacao  <<-->>  \$source_upper = $source_upper "
       ."  -->>  \$val_upper = $val_upper -->> \$array_nome0 count ".count($array_nome0);
exit();
    */

        $_SESSION["anotacao"]=$anotacao;
        $_SESSION["cia_proj"]=mysqli_result($result_anotacao,0,"cip");
        $_SESSION["projeto_autor_nome"]=mysqli_result($result_anotacao,0,"projeto_autor_nome");
        $dados = "<label  style='vertical-align:middle;  cursor: pointer; text-align: left; background-color:#FFFFFF; color:#000000;'  title='N&uacute;mero da Anota&ccedil;&atilde;o' >";
        $dados .= "$anotacao</label>";
        $dados .= "<label for='altera_complementa'  style='cursor: pointer; '  title='Altera ou Complementa o Projeto' >";
        $dados .= "Altera/Complementa (Sim/N&atilde;o)?&nbsp;</label>";
        ...
        //
           ...
           if( $array_altera[0]==1 ) {
                //
    		    //  Quando usar esse Select necessita do GROUP
    	    	/**   $result_altera=mysqli_query("SELECT  a.anotacao, a.cip,a.numprojeto, "
			    *                   ." a.autor,b.nome FROM "
                ...
                */
                $arr_alt1 = 0;
                if( isset($array_altera[1]) ) $arr_alt1 = $array_altera[1];
                //
                $proc="SELECT  a.cia,a.numero,a.projeto,a.autor,a.data,b.numprojeto ";
                $proc.=" FROM $bd_2.anotacao a, $bd_2.projeto b WHERE ";
                $proc.=" a.projeto=$arr_alt1 and b.cip=$arr_alt1  ";
                $proc.=" order by a.numero DESC ";
                $result_altera=mysqli_query($_SESSION["conex"],"{$proc}");
                //
                if( ! $result_altera ) {
                     //
                     $msg_erro .= "Select Tabelas projeto e anotacao - falha: ";
                ...
                /** 
echo "ERRO: LINHA644  -->> \$x_anot = $x_anot  ";
exit();
 */
                ...
                $_SESSION["cia_proj"]=$array_altera[1];
 				if( intval($x_anot)<1 ) {
                    ...
                    echo "<option value='' >== Selecionar ==</option>";
	 		    	for( $x=0; $x<$x_anot; $x++ ) {
                          //
    					  $_SESSION["cia"]=htmlentities(mysqli_result($result_altera,$x,"cia"));  
    					  /**  $_SESSION["numprojeto"]=htmlentities($linha["numprojeto"]);   */ 
                          //
	    				  $_SESSION["numprojeto"]=htmlentities(mysqli_result($result_altera,$x,"projeto"));		
		    			  $nr_anotacao = "Nr. Anota&ccedil;&atilde;o: ".mysqli_result($result_altera,$x,"numero");  
                          $nr_projeto = mysqli_result($result_altera,$x,"numprojeto");
			    		  $autor_nr_proj = "Autor: ".mysqli_result($result_altera,$x,"autor");
                          $dt_anot = htmlentities(mysqli_result($result_altera,$x,"data"));  
                          $dt_anot=substr($dt_anot,8,2)."/".substr($dt_anot,5,2)."/".substr($dt_anot,0,4);
                          $dt_anot = "Data: ".$dt_anot;  
					      //  $autor_nome_proj = "Autor: ".htmlentities(mysqli_result($result_altera,$x,nome));  
                          echo "<option  value=".htmlentities(mysqli_result($result_altera,$x,"numero"))."  "
					            ."  title='$nr_anotacao do Projeto $nr_projeto \r\n$dt_anot \r\n$autor_nr_proj' >"
    					        .mysqli_result($result_altera,$x,"numero")."&nbsp;</option>" ;
	    	              //							
		    	   }  
                   /**  Final - for( $x=0; $x<$x_anot; $x++ ) {  */
                   //
                    ?>
                    </select> 
                   <?php
...
if( $val_upper=="ANOTACAO" ) {
     ...
     */
     ...
     //  require_once("{$_SESSION["incluir_arq"]}cadastrar/form_anotacao.php");
     //
     /**  Caso tenha coautores/coresponsaveis/colaboradores no Projeto/Experimento  */ 
     //  include("{$_SESSION["incluir_arq"]}cadastrar/n_cos.php");
     require_once("{$_SESSION["incluir_arq"]}cadastrar/n_cos.php");
     //     
   	 //  mysqli_select_db($db_array[$elemento]);
     $_SESSION["tabela"]="$bd_2.anotacao";
     /**  
      *    require_once("{$_SESSION["incluir_arq"]}cadastrar/dados_recebidos_arq_ajax.php");
     */
     require_once("{$_SESSION["incluir_arq"]}cadastrar/dds_arq_ajax_anotacao.php");
     //


/**  
echo "ERRO:  LINHA777  -->> \$count_array_temp = $count_array_temp  -->> SEGUE "
          ."<br> \$lennmcps = $lennmcps  E  \$lenvalcps = $lenvalcps  -->> \$arr_nome_val = ".count($arr_nome_val);
exit();
*/
     ...
     //
     // Select/Mysqli - Autor do Projeto 
     $proc="SELECT  anotacao,autor as projeto_autor ";
     $proc.=" FROM $bd_2.projeto  WHERE cip=$projeto  "; 
     $result_anot=mysqli_query($_SESSION["conex"],"{$proc}");
     //
     // if( ! $result_anot ) {
     if( mysqli_error($_SESSION["conex"]) ) {
         //
         /**   die('ERRO: Select $bd_2.projeto e campo anotacao - falha: '.mysqli_error());     */
         $msg_erro .= "Select Tabela projeto e campo anotacao - db/mysql:&nbsp;";
     ...
     //
     /**
     *      CORRIGIDO - function mysqli_result
     *  Local: includes/functions.php
     */
     $_SESSION["anotacao"]=mysqli_result($result_anot,0,"anotacao")+1;
     $_SESSION["projeto_autor"]=mysqli_result($result_anot,0,"projeto_autor");
     //
     if( ! isset($_SESSION["anotacao"]) ) $anotacao="";
     if( isset($_SESSION["anotacao"]) ) $anotacao=$_SESSION["anotacao"];
     //     
     /**  Desativar variavel   */
     if( isset($result_anot) ) {
         //
...
/**  
echo "ERRO:  LINHA821  -->> \$anotacao = $anotacao   "
          ."<br> \$lennmcps = $lennmcps  E  \$lenvalcps = $lenvalcps  -->> \$arr_nome_val = ".count($arr_nome_val);
exit();
 */
    ...
    $dbpri = $db_array[$elemento];
    //
    // Seleciona o banco de dados
    mysqli_select_db($_SESSION["conex"],$dbpri);
    //

  

/**  
 echo  "ERRO: anotacao_cadastrar_ajax/848 -->> ANTES OD INSERT  \$bd_1 =  <b>$bd_1</b> <<- \$bd_2 = <b>$bd_2</b> ->> "
        ." \$source_upper = $source_upper   -- strtoupper(val)==ANOTACAO   -- \$m_array = "
         .gettype($m_array)."<br/>"
        ."   \$campos_nome = $campos_nome <br/> \$campos_valor =  $campos_valor  -->>  \$dbpri = $dbpri  ";
 ...
 */
    ...
    if( $success ) { 
        /**
         *      mysqli_db_query - Esta funcao esta obsoleta, 
         *    nao use esta funcao - Use mysqli_select_db() ou mysqli_query()
         *   UPDATE no campo anotacao da Tabela Projeto do campo numero TB ANOTACAO
         */
        $proc="UPDATE $bd_2.projeto SET anotacao=$anotacao  WHERE  cip=$projeto ";
        $result=mysqli_query($_SESSION["conex"],"$proc");
        if( $result ) {
            //
            ...
            */
            ...
            //
            // Efetiva a transacao nos duas tabelas (anotacao e projeto)
            mysqli_query($_SESSION["conex"],'commit'); 
            //
        } else { 
            //
            /**  Ocorreu ERRO   */
            //  mysqli_error() - para saber o tipo do erro
            $msg_erro .="&nbsp;Anota&ccedil;&atilde;o <b>N&Atilde;O</b> foi cadastrada. Update projeto";
            $msg_erro .= mysqli_error($_SESSION["conex"]).$msg_final;
            //
        ...
        //
        /**  Ocorreu ERRO   */
        $msg_erro .="&nbsp;Anota&ccedil;&atilde;o <b>N&Atilde;O</b> foi cadastrada. Insert anotacao ";
        $msg_erro .= mysqli_error($_SESSION["conex"]).$msg_final;
        //
...
/**    Final - if( $val_upper=="ANOTACAO" ) {  */
//
//  Tabela do ORIENTADOR 
if( $val_upper=="ORIENTADOR" ) {
    //
 ...
 exit();    
   ...
   if ( $res_coord ) { 
        ...
        //  mysqli_error() - para saber o tipo do erro
        $msg_erro .="&nbsp;Orientador n&atilde;o foi cadastrado. ERRO = ".mysqli_error().$msg_final;
         mysqli_query('rollback'); 
         echo $msg_erro;         
   }
   mysqli_query('end'); 
...
/**  Final - if( $val_upper=="ORIENTADOR" ) {  */
//
/**   Tabela do ANOTADOR  cadastrado pelo Orientador   */
if( $source_upper=="ANOTADOR" ) {
    //
    if( $val_upper=="CODIGOUSP" ) {
        //
        /** Definindo variavel String  */
        $strm_arrayup="";
        /** Caso $m_array for String  */
        if( is_string($m_array)) {
           $strm_arrayup=strtoupper(trim($m_array));
        }  
        /**  Final - if( is_string($m_array)) { */
        //
        if( $strm_arrayup=="OUTRO"  ) {
            echo  "usuario,e_mail| | ";   
        } else {
            //
            $elemento=6;
            ...
            //                          
            if( ! $res_pessoa ) {
                //  die('ERRO: Select $bd_1.pessoa - falha: '.mysqli_error());  
                $msg_erro .= "Select Tabela pessoa - db/mysql:&nbsp;".mysqli_error().$msg_final;
                echo $msg_erro;
                exit();        
            }
            echo  'usuario,e_mail|'.mysqli_result($res_pessoa,0,login)."|".mysqli_result($res_pessoa,0,e_mail);
            if( isset($res_pessoa) )  mysqli_free_result($res_pessoa);
        }
        exit();        
    }  
    /**  Final - if( $val_upper=="CODIGOUSP" ) { */ 
    //
}  
/**  Final - if( $source_upper=="ANOTADOR" ) {   */
//
/**  Tabela do anotador  cadastrado pelo Orientador  -  variavel val  */
if( $val_upper=="ANOTADOR" ) {      
    //
     ///  Recebendo dados do FORM e Inserindo
     ///  nas Tabelas ANOTACAO e  PROJETO
    ...
    if( ! $resultado ) {
     ...
     if( $nregs==1 ) {
         $nome_anotador=mysqli_result($resultado,0,"nome");
   ...
   if ( $res_coord ) { 
    ...
    if( ! $resultado ) {
     ...
     if( $nregs==1 ) {
        ...
        ///  mysqli_error() - para saber o tipo do erro
        $msg_erro .="&nbsp;Anotador <b>N&Atilde;O</b> foi cadastrado. ERRO = ".mysqli_error().$msg_final;
         mysqli_query('rollback'); 
         echo $msg_erro;         
   }
   mysqli_query('end'); 
...
if( $val_upper=="PROJETO" ) {
    ...
    */
	...
	///
    ///  Select -  MySQL
    $result=mysqli_query("SELECT  cip,autor FROM $bd_2.projeto WHERE "
    ...
    if( ! $result ) {
        /// die("ERRO: Select Tabela projeto  - ".mysqli_error());
        $msg_erro .= "Select Tabela projeto - db/mysql:&nbsp;".mysqli_error().$msg_final;
        echo $msg_erro;
        exit();        
    }  
    ...
    if( $m_regs>=1 ) {
          ...
          // Verificando se houve erro no Select Tabdla Usuario
          if( ! $result_usu ) {
                //  die("ERRO: Select Tabela usuario   - ".mysqli_error());
                $msg_erro .= "Select Tabela usuario - db/mysql:&nbsp;".mysqli_error().$msg_final;
                echo $msg_erro;
                exit();        
          }  
              ...
              exit();          
                ...
                if ( $success ) { 
                      ...
                      for( $x=0; $x<$count_coresp;  $x++ ) {
                           ...
                           if( !$result ) {
                                mysqli_query('rollback'); 
                                $msg_erro .="&nbsp;CORESP. n&atilde;o foi cadastrado (autor/projeto/coresp):".$m_autor.", ".$_SESSION["numprojeto"].", ".$n_coresponsaveis[$x].mysqli_error().$msg_final;
                                mysqli_query('rollback'); 
                                echo  $msg_erro;
                           }
                     }
                ...
                if( $n_erro==1 ) {
                    ...
                    //  projeto, autor/orientador e numero da anotacao
                    $m_regs=0;
                    $result_proj=mysqli_query("SELECT  cip,autor FROM $bd_2.projeto WHERE "
                                ." trim(fonterec)=trim('".$fonterec."')  and  "
                                ." trim(fonteprojid)=trim('".$fonteprojid."') and "
                    ...
                    if( $m_regs=1 ) {
                         ...
                         if( $res_anotador )  {
                            ...
                            $msg_erro .="&nbsp;Anotador <b>N&Atilde;O</b> foi cadastrado.".mysqli_error().$msg_final;
                            echo $msg_erro;                                   
                            exit();
                         }                   
                    } else {
                         $msg_erro .="&nbsp;Projeto <b>N&Atilde;O</b> encontrado.".mysqli_error().$msg_final;
                         echo $msg_erro;                                   
                         exit();                          
                    }
                }
...
if( $val_upper=="PESSOAL" ) {
    ...
    */	
    ...
    if ( $arr_nome_val['codigousp']==0) {
        ...
        if( ! $result ) {
            mysqli_free_result($result);          
            die("Falha erro no Select/Atribuir codigoUSP".mysqli_error());
        }
        $m_regs=mysqli_num_rows($result);
        if ($m_regs>0) {
	...
	if( ! $result_usu ) {
		  ...
		  die("Falha erro no Select".mysqli_error());
	}
    $m_regs=mysqli_num_rows($result_usu);
    mysqli_free_result($result_usu);
    if(  intval($m_regs)>=1 ) {
        $msg_erro .= "&nbsp;Esse C&oacute;digo:&nbsp;".$arr_nome_val['codigousp'];
        $msg_erro .= " j&aacute; est&aacute; cadastrado.".$msg_final;
        echo $msg_erro;
    } else {
        //
...
if( $val_upper=="USUARIO" ) {
    ...
    if ( ! $result_usu ) {
        $msg_erro .= "&nbsp;Usu&aacute;rio:&nbsp;".$arr_nome_val[login]." - falha no mysql/query:".mysqli_error().$msg_final;
         echo $msg_erro;
         exit();
    }
    $m_regs = mysqli_num_rows($result_usu);
    ...
    if( intval($m_erro)<1 ) {
        ...
        if( ! $res_email ) {
            ...
            die("ERRO: Select pessoa campo e_mail falha: ".mysqli_error());
        }  else {
            $usr_email=html_entity_decode(trim(mysqli_result($res_email,0,'e_mail')));
            $data['senha'] = $arr_nome_val['senha'];
            mysqli_freeresult($res_email);
            //
