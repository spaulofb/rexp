	 ...
	 if( intval($n_cpo) <= intval($cpo_final) ) {  
         ...
         */
         ...
         if( is_array($m_array) ) {
             ...
             /**
              if( $val_upper=="GEMAC" ) {
                  echo "ERRO: ANTES -->>  1) \$cpowh = $cpowh  <<-->> \$val = $val -->> \$m_array (ARRAY)  <br> ";     
              }
                */
             ...
             // if( $val_upper=="USP" ) {
              ...
              //  if(strtoupper($cpowh)=="SALATIPO" ) {    
             //   if(strtoupper($pricpotbant)=="BLOCO" ) {   
             /**
             if(strtoupper($cpowh)=="INSTITUICAO" ) {    
                   //
                  //  echo "ERRO:   1) \$tbant = $tbant  <<-->> \$cpowh = $cpowh  <<-->>"
                  echo "  1) \$tbant = $tbant  <<-->> \$cpowh = $cpowh  <<-->>"
                      ." \$val = $val <br> -  \$pricpotbant = $pricpotbant - \$m_array (ARRAY)  <br> ";     
                    //  exit();
              }
              */
              ...
              */
              ...
              if( $xkey ) {
                ...
                /**  
                  $strmarr=implode(" ",$cparr);
                  echo "  1) DENTRO \$zkey = $zkey <<--   \$cpowh_up  = $cpowh_up  <<-- \$_SESSION[select_cpo] = {$_SESSION["select_cpo"]} <<--"
                   ."<br> -->> \$cpowh = $cpowh  <<- \$pricpotbant = $pricpotbant "
                  ."<br> ->> \$val = $val -->>  \$cparr  ARRAY ";     
                    exit();
                    */
             ...
             /**
              if( $val_upper=="GEMAC" ) {
                  $strmarr=implode(" ",$m_array);
                  echo "ERRO:  -->>  2) \$_SESSION[select_cpo] = {$_SESSION["select_cpo"]} <<-->> \$cpowh = $cpowh  <<-"
                  ."<br> ->> \$val = $val -->>  $strmarr\r\n  \$m_array  gettype = ".gettype($m_array);     
                  exit();
              }
              */
            ...
            //  if( strtoupper($cpowh)=="INSTITUICAO" ) {                  
              ...
              /**
               if( strtoupper($cpowh)=="UNIDADE" ) {                      
                    $strmarr=implode(" ",$m_array);
                    echo "  2) Continuar \$xkey = $xkey <<--   \$cpowh_up  = $cpowh_up - <br>"
                        ." \$_SESSION[select_cpo] = {$_SESSION["select_cpo"]} <<--"
                         ."<br> -->> \$cpowh = $cpowh  <<- \$pricpotbant = $pricpotbant "
                        ."<br> ->> \$val = $val -->>  $strmarr\r\n  \$m_array  ARRAY ";     
              }
              */
    ...
    //      $_SESSION['cponm1'][$chave]
         ...
         //
          ...
          /**    
              if( strtoupper($cpowh)=="UNIDADE" ) {                      
          if(strtoupper($pricpotbant)=="BLOCO" ) {                        
                  echo "  3) \$i = $i - \$_SESSION[select_cpo] = {$_SESSION["select_cpo"]} - <br>"
                     ."  - \$cpowh = $cpowh ($cpowh_up) - \$table_atual = $table_atual -->> \$tb_up =$tb_up   "
                     ."<br> ->> \$val = $val -<br> \$m_array  gettype = ".gettype($m_array);     
      ...
      /**  if( $cpowh_up=="INSTITUICAO" && strtoupper(trim($table_atual))=="SALATIPO"  ) {  */
         ...
         //
         /**  DESATIVADO $_SESSION['cponm1']  */
//         $_SESSION['cponm1'][$chave]=$pricpotbant;
  
         // $_SESSION['cponm1'][$chave]=$select_cpo;
  
//         $_SESSION['cpoval1'][$chave]=$val;
         //
...
*/              
         ...
         $cp_cpowh=$cpowh;
         //
         if( strtoupper($tb_up)=="INSTITUICAO" ) $cp_table_atual="instituição";
         if( preg_match("/instituicao|codinst|idinst/ui",$cpowh_up) ) { 
                 $cp_cpowh="Instituição";
         }
         if( strtoupper($tb_up)=="DEPTO" ) $cp_table_atual="departamento";
...
exit();     
   */  
     ...
     if( $success ) { 
         ...
         //
         $msg_erro .="<span style='font-size: large;padding:.8em 0 .8em 0;'>";
         $msg_erro .="$nomepessoa n&atilde;o foi cadastrado.</span>".$msg_final;
         echo $msg_erro;         
         //
     }
     // 
