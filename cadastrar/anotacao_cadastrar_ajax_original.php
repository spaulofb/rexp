<?php
///   Arquivo para CADASTRAR ANOTACAO - 20170918
///
...
if( $source_upper=="ANOTACAO" ) { ////  IF  - Tabela ANOTACAO 
    ...
    if( strtoupper(trim($val))=="PROJETO" ) {
           ...
           $result_anotacao=mysqli_query("SELECT  a.anotacao,a.cip,a.numprojeto,b.nome as projeto_autor_nome "
           ...
           echo  utf8_decode($dados)."<label".$xz;
            ...
            if( $array_altera[0]==1 ) {
    	    	...
    	    	/*	$result_altera=mysqli_query("SELECT  a.anotacao, a.cip,a.numprojeto, "
								   ...
								   ." order by a.cip ");	  
                */
                $result_altera=mysqli_query("SELECT  a.cia,a.numero,a.projeto,a.autor,a.data,b.numprojeto "
...
if( strtoupper($val)=="ANOTACAO" ) {
	...
	$result_anot=mysqli_query("SELECT  anotacao,autor as projeto_autor FROM $bd_2.projeto  WHERE cip=$projeto  ");
    ///
    if( ! $result_anot ) {
