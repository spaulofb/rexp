...
echo "ERRO: tabela_anotador_selecionada/70 --  TUDO CERTO --- \$usuario_conectado = $usuario_conectado ";
exit();
...
if( intval($total_regs)<=0 ) {    
    ...
    $num_fields=mysqli_num_fields($query);  //  Obtem o n?mero de campos do resultado
    ...
    $campos_fora = array("ARQUIVO","DATA","PROJETO_TITULO","AUTOR");
    ...
    while( $linha = mysql_fetch_row($query)) {
       ...
       <tr align="left" valign="top"  >
        ...
        for( $column_num=0; $column_num<$num_fields; $column_num++) {
            ...
            //  if( $field_name_upper=='T?TULO' ) {
            if( stripos($field_name_upper,'T?TULO_ANOT')>-1  ) {
               ...
               // $valor = substr($valor,strpos($valor,"_")+1,strlen(trim($valor)));
                ...
                <?php
                  $cmdhtml = "<a href='#' onclick='javascript: $m_function(\"DESCARREGAR\",\"$valor\",\"$usuario_conectado#anotacao\");return true;'  "
                        ."  id='relattext'  class='linkum'   "
                        ."  title='Clicar'  style=' text-align: center; vertical-align:top; line-height:normal;' >";  
                  $cmdhtml .=$titulo."</a>";
                  $cmdhtml .="</td>";
                  echo $cmdhtml;
                ...
                ?>                        
                <td style="text-align: <?php echo $text_align;?>; white-space:nowrap; font-weight: bold; border: 1px solid #000000; <?php echo $font_size_family;?> ">                
                <?php  echo $valor;?></td>
                <?php                
            }
        }
   ...
   // Calculando pagina anterior
    ...
    if( $pgs > 1 ) {
        ...
        echo  '<table border="0"  cellpadding="1"  cellspacing="0"  align="center" style="margin-top: 0px; padding-top: 0px; width: 100%;" >';    
        ...
        if( $mais<=$_SESSION["numero_de_pags"] || ($inicio+$maximo)<$total_regs ) {
            //  echo "<a href=\"?pagina=$mais&seed=$seed\" class='texto_paginacao'>pr&oacute;xima</a>";
            // echo "<a href=\"$pagina_atual?pagina=$mais\" class='texto_paginacao'>pr&oacute;xima</a>";
            //  echo "<a href=\"?pagina=$mais\" class='texto_paginacao'>pr&oacute;xima</a>";
