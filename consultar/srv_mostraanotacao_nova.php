...
if( preg_match("/^TODOS|ordenar/i",$opcao_maiusc) )  {
        ...
        ***/
        ...
        if( ! $result_consult_anotacao ) {
             /*  $msg_erro .= "Criando a Tabela {$_SESSION["table_consultar_anotacao"]}  - Falha: ".mysqli_error($_SESSION["conex"]).$msg_final;
               echo $msg_erro; */
              echo $funcoes->mostra_msg_erro("Criando a tabela {$_SESSION["table_consultar_anotacao"]} -&nbsp;db/mysql:&nbsp;".mysqli_error($_SESSION["conex"]));            
              exit();        
        } 
     ...
     /***
         $msg_erro .= " srv_mostraanotacao.php/122 -  \$resultado_outro = $resultado_outro ";
        echo  $msg_erro;
     ***/
     exit();     
     ///
    ...
    ///  Selecionando Projeto
     ...
     if( ! $resultado_projeto ) {
         ...
         /* $msg_erro .= "Selecionando Tabela projeto  - db/mysql:&nbsp; ".mysqli_error($_SESSION["conex"]).$msg_final;
             echo $msg_erro; */
         echo $funcoes->mostra_msg_erro("Selecionando Tabela projeto  -&nbsp;db/mysql:&nbsp;".mysqli_error($_SESSION["conex"]));
         exit();
     }         
    ...
    $campos = "&nbsp; srv_mostraanotacao.php/192 -   \$numprojeto = $numprojeto -- \$titulo_projeto  = $titulo_projeto <br> ";
    $campos .= "\$autor_projeto =  $autor_projeto - \$data_projeto = $data_projeto  -- \$anotacao = $anotacao ";       
     ...
     if( ! $resultado_anotacao ) {
          ...
          echo $msg_erro;  */
          echo $funcoes->mostra_msg_erro("Selecionando Anota&ccedil;&atilde;o $anotacao do Projeto: $numprojeto -&nbsp;db/mysql:&nbsp;".mysqli_error($_SESSION["conex"]));            
          exit();
     } 
     ///  Definindo os nomes dos campos recebidos do MYSQL SELECT - mysql_fetch_array
     ...
     if( strlen(trim($testemunha1))>=1 or strlen(trim($testemunha2))>=1  ) {
         ...
         if( ! $res_testemunhas ) {
             ...
             echo $msg_erro;  */
             echo $funcoes->mostra_msg_erro("Selecionando testesmunhas da Anota&ccedil;&atilde;o -&nbsp;db/mysql:&nbsp;".mysqli_error($_SESSION["conex"]));
             exit();
         }        
         $testemunhas="";
         /////  Numero de registros
         ...
         for( $ntest=0 ; $ntest<$num_regs ; $ntest++ ) {
              ...
              $testemunhas .="<p style='text-align:justify;font-size: medium;;margin:0;padding:0; margin-left:30%;'>"
     ...
     $confirmar0 .="<b>T&iacute;tulo do Projeto</b>:<br>"                
                 ."$titulo_projeto</p>";
     $confirmar0 .="<p style='text-align:center;font-size: medium;'>"
     ...
     $confirmar0 .="<p style='text-align:center;font-size: medium; overflow: auto;'>"
                 ."<b>T&iacute;tulo da Anota&ccedil;&atilde;o</b>:<br>"                
                 ."$titulo_anotacao</p>";                                     
     $confirmar0 .="<p style='text-align:center;font-size: medium;  margin-top:4px;'>"
