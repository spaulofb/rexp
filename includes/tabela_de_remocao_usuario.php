...
/*
    Exibindo a Tabela Usuario - Remover
*/
$campos_query = "*";
...
if( intval($total_regs)<=0 ) {
    ...
    if( ! $query ) {
        $msg_erro .=utf8_decode("Sem resultado - Select na tabela tempor&aacute;ria $table_rm_usu -&nbsp;db/mysql:&nbsp;").mysql_error().$msg_final;  
        echo $msg_erro;
        exit();            
    }
    ...
    for( $i = 0;$i<$num_fields; $i++) { //  Pega o nome dos campos
     ...
     $sqlcmd="SELECT ".$max_length." FROM   $table_rm_usu  ";
     $result_max_length = mysql_query($sqlcmd);          
     ///
     if ( ! $result_max_length ) {
          $msg_erro .=utf8_decode("Select maximo tamanho dos campos da tabela tempor&aacute;ria  $table_rm_usu -&nbsp;db/mysql:&nbsp;").mysql_error().$msg_final;  
          echo $msg_erro;
         exit();                      
     }    
...
///    echo "<table>";
    ...
    for($column_num = 0; $column_num < $num_fields; $column_num++) {
            ...
            ///
            echo "<th  class='font_size_family' style='text-align: $text_align; background-color: #00FF00; border: 1px solid #000000;' >".ucfirst($field_name)."</th>";
            ///
    }
    echo "</tr>";
    ...
    while( $linha = mysql_fetch_row($query)) {
        ...
        for( $column_num=0; $column_num<$num_fields; $column_num++) {
         ...
         ///   if( strtoupper($field_name)=='ARQUIVADO_COMO' ) {
            ...
            ///
            if( $field_name_upper=='COD' )  $m_relatext=$linha[$column_num];  
            ////  if( $field_name_upper=='NOME' )  $nome=$linha[$column_num];  
            /// Nome do Usuario para ser removido
            if( ! $nome=trim(mysql_result($query,$conta_linha,"nome")) ) {
                    $nome="";                
            }                                     
           ?>    
          <td  style="text-align: <?php echo $text_align;?>; white-space: nowrap; padding: .3em; font-weight: bold; border: 1px solid #000000;" >
          <?php  
             echo  "<a href='javascript: $m_function(\"REMOVER\",\"$m_relatext\",\"$conjunto#@=$selecionado#@=$nome\")' "
