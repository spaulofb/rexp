...
if( strtoupper($source)=="SAIR" ) {
	...
	exit();
	#
} elseif( strtoupper(trim($source))=="CAMPOS_OBRIGATORIOS" )  {    
  ...
  /*      
    ...
    */
       ...
       if( ! $result_usu ) {
           ...
           die('ERRO: SELECT usuario/orientador: '.mysqli_error($_SESSION["conex"]));
           exit();  
       }
       $n_regs = 0;
       $n_regs = mysqli_num_rows($result_usu);
       ...
       if( $n_regs>=1 ) {
           ...
           if( $aprovado==1 ) {
                $confirmar0 ="<p style='text-align:center;font-size: medium;'>"
                       ."<b>J&aacute; est&aacute; cadastrado como Orientador</b>.<br>"
                       ."Para acessar, basta fazer o seu login.</p>";
                $confirmar1 .=$confirmar0."<div style='width: 100%; text-align: center;'>";            
                if( strtoupper($sexo)=="M" ) $texto = "J? est? cadastrado e aprovado como Orientador";
                if( strtoupper($sexo)=="F" ) $texto = "J? est? cadastrada e aprovada como Orientadora";
           ...
           //           
           $msg_ok .= "<span style='color: #000000;margin-left: 2px; padding-left: 4px;' >"
                       ."<br>&nbsp;Nome: <b>$nome</b> "
                       //  ."<br><br>CPF: $cpf"
                       ."<br><br>&nbsp;C&oacute;digo/USP: <b>$codigousp</b> "
                       ."<br><br>&nbsp;Usu&aacute;rio/Login: <b>$login</b> "
                      ."<br><br>&nbsp;E_Mail: <b>$e_mail</b> </span>";  
...
if( strtoupper($val)=="ORIENTADOR_NOVO" ) {
    ...
    if(  $m_regs<1 ) {
         ...
         $array_email = array("EMAIL","E_MAIL","USER_EMAIL","EMAIL_USER");
         //  Definindo os nomes dos campos recebidos do FORM
         // //  while ( $arr_nome_val = mysqli_fetch_array($result_projeto,MYSQL_ASSOC) ) { 
              foreach( $arr_nome_val   as $chave => $valor )  { 
                   $campo_nome = strtoupper($chave);
                   if( in_array($campo_nome,$m_usuario_arr) ) {
                      $login = trim($valor);
       ...
       /*   $result_login = mysqli_query("SELECT a.login,a.aprovado,b.nome  FROM  usuario where "
                        ."  upper(trim(login))='$upper_login' ");
         */ 
          $result_login = mysqli_query("SELECT a.login,a.aprovado,b.nome,b.e_mail  "
                        ." FROM pessoal.usuario a, pessoal.pessoa b where "
                        ."  upper(trim(a.login))='$upper_login' and a.codigousp=b.codigousp  ");
           //
          if ( ! $result_login ) {
                 mysql_free_result($result_login);
                $msg_erro .= "&nbsp;Usu&aacute;rio/Login:&nbsp;".$login." - falha no mysql/query:".mysqli_error($_SESSION["conex"]).$msg_final;
                echo $msg_erro;
                exit();
          }
          $m_regs = mysqli_num_rows($result_login);
          if(  $m_regs>=1 ) {
              $aprovado=mysql_result($result_login,0,"aprovado");
              $nome=mysql_result($result_login,0,"nome"); 
              $e_mail=mysql_result($result_login,0,"e_mail"); 
              if( $aprovado==1  ) {
                  $sn_aprovado = "&nbsp;Usu&aacute;rio/Login:&nbsp;".$login."&nbsp;<b>j&aacute; est&aacute; Cadastrado"
                              ." e Aprovado</b> com o Nome: $nome e o E_mail: $e_mail";  
              }  elseif( $aprovado<1 ) {
                  $sn_aprovado = "&nbsp;Usu&aacute;rio/Login:&nbsp;".$login."&nbsp;<b>j&aacute; Cadastrado mas ainda "
          ...
          //  Verificando cadastro na tabela pessoa - nome ou e_mail - MYSQL  replace
          $res_email=mysqli_query("SELECT codigousp,nome,e_mail FROM pessoal.pessoa where "
                  ." replace(upper(trim(nome)),'  ',' ')='$pessoa_nome' or  upper(trim(e_mail))='$upper_email' ");
          //        
          if( ! $res_email ) {
          ...
          if( $lnregs==1 ) {
               $dup=$lnregs;
               $msg_erro .= "J&aacute existe cadastro com Nome: ".trim($nome)." OU E_Mail: ".trim($e_mail);
          } elseif( $lnregs>1 ) {
               $dup=$lnregs;
               $msg_erro .= "J&aacute existem pessoas cadastradas com mesmo Nome OU E_Mail";
          } 
          mysql_free_result($res_email);
          if( $dup>=1 ) {
               print $msg_erro;
          ...
          */
          ...
          //  Execute the queries          
          //  mysql_db_query - Esta funcao e obsoleta, nao use esta funcao - Use mysql_select_db() ou mysqli_query()
          mysqli_query("LOCK TABLES ".$_SESSION['tabela']." WRITE, pessoal.usuario WRITE  ");
          ...
          $res_pessoa .="  (codigousp,cpf,passaporte,nome,sexo,categoria,instituicao,unidade,depto,setor,bloco,sala,salatipo,fone,ramal,e_mail)  "
           ...
           if( $sqlcmd ) { 
               ...
               $res_login= "insert into ".$_SESSION['tabela']." (login,codigousp,pa) "
                                  ." values('$login',$codigousp,$pa) "; 
               //                  
               ...
               if( $sqlcmd ) { 
                    ...
                    $commit = "rollback";
                    $msg_erro .="&nbsp;Falha Tabela usuario insert - ".mysqli_error($_SESSION["conex"]).$msg_final;
              ...
              //  mysqli_error($_SESSION["conex"]) - para saber o tipo do erro
              $commit = "rollback";
              $msg_erro .="&nbsp;Falha Cadastrar Orientador $nome - ".mysqli_error($_SESSION["conex"]).$msg_final;
              //  mysqli_query('rollback'); 
              echo $msg_erro; 
              $lnerro=1;        
           }           
           ...
           if( $lnerro<1 ) {
               ...
               Email do novo Orientador: $e_mail \n<br>
               ...
               *** não responda a este EMAIL ****
               ";
            //   mail($aprovador_email, stripslashes(utf8_encode($assunto)), $message,$headers1);
               
   /*     $msg_erro .= "\$a_link = $a_link  ";
            $msg_erro .= "<br> \$corpo = $corpo - <br>"
            ."  \$login = $login  -- \$descr_categ = $descr_categ - <br> \$e_mail = $e_mail - "
            ." \$activation_code = $activation_code  -  \$fone = $fone  ";  
           echo  $msg_erro;
           exit();
     */
...
if( strtoupper(trim($source))=="ANOTADOR" ) {
