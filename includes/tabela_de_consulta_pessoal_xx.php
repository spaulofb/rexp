   ...
   ///    echo $_SESSION["titulo"];
    ...
    while( $linha = mysql_fetch_row($query)) {
        ...
        for( $column_num=0; $column_num<$num_fields; $column_num++) {
            ...
            ***/
           ?>
            <td class="font_size_family" >
               <?php  
                   ///
                    echo "<a href='javascript: $m_function(\"MOSTRAR\",\"$m_relatext\")' "
                          ." id='m_selecionado' class='link_href' title='Clicar para selecionar Pessoa' >";
                    ///
                   /* IMPORTANTE: Detectar codificacao  de caracteres  - 20171005   */
                   $codigo_caracter=mb_detect_encoding($valor);
                   if( trim(strtoupper($codigo_caracter))!="UTF8" ) {
                       /// echo  htmlentities($valor,ENT_QUOTES,"UTF-8");
                        echo  $valor;
                       ////
                   } else {
                        echo  "$valor;    
