...
if( ! isset($_SESSION["arquivo_projeto"]) ) {
   ...
   $msg_erro .= "SESSION arquivo_projeto não está ativa.".$msg_final;   
   echo $msg_erro;
   exit();
} else {
   /// 
...
/// echo "ERRO:  LINHA/86 --- \$result = $result  <<<---  {$_SESSION["arquivo_projeto"]}  <<<--->>>   \$caminho_arquivo =  $caminho_arquivo --->>>- \$file = $file   ".$_GET["file"];
///  exit();

////  Verifica se o arquivo EXISTE  -  alterado 20180926
