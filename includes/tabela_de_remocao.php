...
if( $total_regs<=0 ) {
    ...
    for($column_num = 0; $column_num < $num_fields; $column_num++) {
            ...
            if( strtoupper(trim($field_name))=="OBJETIVO_DESCRICAO" ) $field_name="Objetivo";
            if( strtoupper(trim($field_name))!=="CIP" ) {
                echo "<TH ALIGN='LEFT' "
    ...
    while( $linha = mysql_fetch_row($query)) {
        ...
        for( $column_num=0; $column_num<$num_fields; $column_num++) {
            ...
            if( strtoupper($field_name)=='ARQUIVADO_COMO' ) {
                ...
                $valor=htmlentities(trim(mysql_result($query,$conta_linha,"arquivado_como")));                                   
                $valor = substr($valor,strpos($valor,"_")+1,strlen(trim($valor)));      
                $m_relatext=$linha[$column_num]; $sem_link=1;
            } else {
                $sem_link=0;
                if( strtoupper($field_name)=='T?TULO' ) {
                    ...
                    $valor=trim($projeto_titulo_parte); 
                } else $valor=$linha[$column_num];                                           
            }
            //  if( $sem_link==1 ) {
           //  }
           if( strtoupper($field_name)!=='CIP' ) {
               ?>    
              <td  style="white-space:nowrap; font-weight: bold; border: 1px solid #000000; <?php echo $font_size_family;?>" onmouseover="javascript: window.status='';return true;" >
              <?php
    /*          
              echo   "<a  href='#' onclick='javascript: $m_function(\"REMOVER\",\"$m_relatext\",\"$conjunto#@=$selecionado\")' "
                              ." id='m_selecionado' class='link_href' "
                              ." title='Clicar para remover'   onmouseover='javascript: window.status=\"\";'  >";                
      */
              $cip=mysql_result($query,$conta_linha,"cip");                                   
              echo   "<a  href='#' onclick='javascript: $m_function(\"SELECIONADO\",\"$cip\")' "
                          ." id='m_selecionado' class='link_href' "
                         ." title='Clicar para remover'   onmouseover='javascript: window.status=\"\";'  >";                
              echo $valor;
              echo "</a>";
